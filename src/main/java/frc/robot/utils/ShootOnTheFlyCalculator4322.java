...
public class ShootOnTheFlyCalculator4322 {
   ...
   */
  public ShootOnTheFlyCalculator4322(InterpolatingTreeMap<Double, ShooterParams> shooterTable) {
    this.shooterTable = shooterTable;
  }

  /** Creates a calculator with a new empty table. Use {@link #addTableEntry} to populate. */
  public ShootOnTheFlyCalculator4322() {
    this.shooterTable =
  ...
  private ShooterCommand calculateBothAdjustments(
    ...
    we can look for the roots of f(x) = Vel(x) - requiredVelocity to get us our shooter params, 
    where Vel(x) = x/Tof(x)
