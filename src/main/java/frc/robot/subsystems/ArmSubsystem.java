...
public class ArmSubsystem extends SubsystemBase {
    ...
    public final Angle armStartupOffset = Degrees.of(startDegrees);
    ...
    public ArmSubsystem(boolean showData) {
        ...
        if (showTelemetry) {
            ...
            SD.sd2("Arm/Values/poscf", positionConversionFactor);
    ...
    public void position() {
        ...
        armClosedLoopController.setSetpoint(
 ...
 public Command jogIntakeArmCommand(DoubleSupplier jogRate) {
    return new FunctionalCommand(
        ...
        () -> {
          if (getAngle().lt(maxAngle) && jogRate.getAsDouble() > 0)
            armMotor.setVoltage(jogRate.getAsDouble() * RobotController.getBatteryVoltage());
          else if (getAngle().gte(minAngle) && jogRate.getAsDouble() < 0)
            armMotor.setVoltage(jogRate.getAsDouble() * RobotController.getBatteryVoltage());
          else
            armMotor.set(0);
        }, // execute
    ...
    public void runAtVelocity(double radiansPerSecond) {
        armClosedLoopController.setSetpoint(radiansPerSecond, ControlType.kVelocity, ClosedLoopSlot.kSlot1);
    }
